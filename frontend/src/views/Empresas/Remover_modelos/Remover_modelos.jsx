...
function Remover_modelos() {
  ...
  useEffect(() => {
    document.title = "Remover | Modelos"
    pegarIdAnimais()
  }, [])
  ...
  async function selectModelos(marca_id) {
    try {
      const resposta = await fetch(process.env.REACT_APP_URL_API + "/empresasModelos/" + idEmpresa + "/marca/" + marca_id)

      const dados = await resposta.json()
      console.log(dados)

      setModelos(dados.map(value => {
        return { value: value.modelo_id.toString(), label: value.modelos.nome }
      }))
    } catch (error) {
  ...
  async function removerModelos() {
    ...
    try {
      const result = await fetch(process.env.REACT_APP_URL_API + "/empresasModelos/" + idEmpresa, {
        method: "DELETE",
        headers: {
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="row justify-content-center border rounded-4 bg-light shadow-sm mb-5 bg-body-tertiary rounded col-12 col-md-8 position-absolute top-50 start-50 translate-middle ">
            ...
            <div className="form-floating mb-3 mb-md-4">
              <select
                ...
                onChange={(e) => {
                  ...
                  pegarEmpresasMarcasPorIdEmpresaIdProduto(e.target.value)
                }}
                className="form-select"
                id="floatingSelect"
                aria-label="Floating label select example">
          ...
          <div className="imgRemoverModeloEmpresa col-md-6 d-flex mt-3 mt-md-0 rounded-4 p-3 justify-content-center">
